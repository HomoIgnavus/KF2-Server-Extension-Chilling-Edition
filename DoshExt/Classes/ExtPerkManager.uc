...
{
	local int i;
	// `log("ExtPerkManager.ApplyPerkClass() Player=" @ PlayerOwner @ ", PerkClass=" @ P);
...
function bool ApplyPerkName(string S)
{
	...
	for (i=0; i<UserPerks.Length; ++i)
		...
		{
			// `log("ExtPerkManager.ApplyPerkName() Applying perk: " @ S @ " at index " @ i);
			ApplyPerk(UserPerks[i]);
			return true;
		}
	return false;
...
function ApplyPawnStats()
{
	...
	if (HP == None)
	{
		SetTimer(0.2, false, 'ApplyPawnStats');
		return;
	}
...
{
	// `log("overrided ModifyArmor called by " @ GetScriptTrace());
	MaxArmor = 255;
	// if (CurrentPerk!=None)
	// 	CurrentPerk.ModifyArmor(MaxArmor);
}

function ModifyArmorInt(out int MaxArmorInt)
{
	if (CurrentPerk!=None)
